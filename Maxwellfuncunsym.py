...
def Maxwellfuncunsym(T,N,tau,h,m,rho,L,tolgmres,eps,mu,sig,theta,alpha,Ce ):#,J,m0,E0,H0):,X,Y,trace_space,trace_matrix
    print('h is ', h, ' N is', N)
    start = timeit.default_timer()
    bempp.api.global_parameters.assembly.boundary_operator_assembly_type = "dense"
    # OrderQF = 1
    # tol = 10 ** (-8)
    # bempp.api.global_parameters.hmat.eps = 1.0#10 **
    # #bempp.api.global_parameters.hmat.max_block_size=10
    # #bempp.api.global_parameters.hmat.max_rank = 4
    # #bempp.api.global_parameters.hmat.min_block_size = 1
    # #bempp.api.global_parameters.hmat.cutoff = 1.7*10**15
    # #bempp.api.global_parameters.assembly.interpolation_points_per_wavelength=10
    ...
    timeoutput = False  # time measurements
    plotoutput = False
    ...
    J = project(Expression(['1.00', '-1.0', '3.0'], degree=1), X)
    ...
    for j in range(0, int(N)):  # time stepping: update from timestep t_j to j+1
        ...
        for kk in range(1, j + 2):  # Convolution, start bei 0, da psiko(0)=0 aber gamma_TE vlt nicht...
        ...
        t = (j + 1) * tau
        Rhs = np.concatenate(
                [eps / tau * M1.weak_form().dot( Ejko) - M1.weak_form() * J.vector()*(tau*j)  - G1T.dot(boundary_rhs[nBEM:]),
                 mu / tau * M0.weak_form().dot(Hjko), boundary_rhs[:nBEM]])
        #else:
        #    Rhs = np.concatenate(
        #        [eps / tau * M1.weak_form() * Ejko  - G1T.dot(boundary_rhs[nBEM:]),
